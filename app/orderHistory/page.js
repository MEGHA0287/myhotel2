...
const OrderHistoryList = () => {
    ...
    const handleSearch = () => {
        ...
        const filteredData = orderHistoryWithDifference.filter((historyItem) => {
            const historyItemDate = new Date(historyItem.timestamp).toISOString().split('T')[0];
    ...
    const handleExportToExcel = () => {
        // Create a worksheet
        const worksheet = XLSX.utils.table_to_sheet(document.getElementById("orderHistoryTable"));
    ...
    const handlePrint = () => {
        const printContent = orderHistoryWithDifference.map((historyItem) => ({
            ...
            items: historyItem.updatedFields?.items.map((item) => `${item.name} X ${item.quantity}`),
    ...
    const orderHistoryWithDifference = orderHistory.map(
        (historyItem, index, array) => {
            ...
            const filteredItem = filteredOrders.find((filtered) => filtered.details?._id === historyItem._id);
            ...
            if (filteredItem && filteredItem.details) {
                ...
                const filteredAmount = Math.round(filteredItem.details.updatedFields?.total);
    ...
    return (
        <>
            ...
            <div className="container mx-auto mt-10 p-2 bg-white rounded-md shadow-md font-sans">
                ...
                <div className=" justify-center mx-auto max-w-5xl">
                    <table className="border-collapse border border-gray-300 w-3/4 divide-y divide-gray-200" id="orderHistoryTable">
                        ...
                        <tbody className="text-center">
                            {filteredOrders.length > 0 ? (
                                ...
                                filteredOrders.map((historyItem, index) => {
                                    const orderNumber = historyItem.orderNumber.replace(/\D/g, "");
                                    const isFirstOccurrence = !encounteredOrderNumbers.has(orderNumber);

                                    // Determine the background color based on the difference
                                    const bgColorClass = historyItem.difference < 0 ? "bg-red-100 text-red-600" :
                                        (isFirstOccurrence ? "bg-green-100 text-green-700" : "bg-white text-gray");
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="border border-gray-300 pl-2">{orderNumber}</td>
                                            <td className="border border-gray-300">
                                                {new Date(historyItem.timestamp).toLocaleDateString("en-GB")}
                                            </td>
                                            <td className="border border-gray-300 px-1 py-1">
                                                {historyItem.updatedFields?.items?.map((item, itemIndex) => (
                                                    <div key={itemIndex}>
                                                        <span>
                                ...
                                orderHistoryWithDifference.map((historyItem, index) => {
                                    ...
                                    return (
                                        <tr
                                            ...
                                            className={bgColorClass}
                                            >
                                            <td className="border border-gray-300 px-1 py-1">{orderNumber}</td>
                                            <td className="border border-gray-300 px-1 py-1">
                                                {new Date(historyItem.timestamp).toLocaleDateString("en-GB")}
                                            </td>
                                            <td className="border border-gray-300 px-1 py-1">
